...
const DonationForm = ({ student }) => {
  ...
  const validationSchema = Yup.object({
    ...
    phoneNumber: Yup.string().when('paymentMethod', {
      is: 'mpesa',
      then: (schema) => schema.required('Phone number is required').matches(/^254[0-9]{9}$/, 'Enter valid number (254XXXXXXXXX)'),
  ...
  const handleSubmit = (values) => {
    ...
    // Update localStorage students
    const allStudents = JSON.parse(localStorage.getItem('students') || '[]');
    const updatedStudents = allStudents.map(s => 
      s.id === student.id 
        ? { 
            ...s, 
            amount_raised: (s.amount_raised || 0) + donationAmount,
            supporters_count: (s.supporters_count || 0) + 1
          }
        : s
    );
    localStorage.setItem('students', JSON.stringify(updatedStudents));
    
    // Update dummy students in localStorage for persistence
    const storedDummyStudents = JSON.parse(localStorage.getItem('dummyStudents') || JSON.stringify(dummyStudents));
    const updatedDummyStudents = storedDummyStudents.map(s => 
      s.id === student.id 
        ? { 
            ...s, 
            amount_raised: (s.amount_raised || 0) + donationAmount,
    ...
    localStorage.setItem('dummyStudents', JSON.stringify(updatedDummyStudents));
    
    // Check if student has reached or exceeded their goal
    const currentRaised = student.amount_raised || 0;
    const goalAmount = student.fee_amount || 0;
    const newAmountRaised = currentRaised + donationAmount;
    const isGoalReached = newAmountRaised >= goalAmount;
    
    console.log('Donation check:', {
      currentRaised,
      goalAmount, 
      donationAmount,
      newAmountRaised,
    ...
    // Show transfer option only if there are excess funds
    if (isGoalReached) {
      const excess = newAmountRaised - goalAmount;
      console.log('Goal reached! Excess amount:', excess);
      ...
      if (excess > 0) {
        ...
        if (otherStudents.length > 0) {
          ...
          setAvailableStudents(otherStudents);
  ...
  const handleTransfer = (selectedStudent) => {
    ...
    setDonationSuccess(true);
    alert(`✨ KSh ${excessAmount.toLocaleString()} has been transferred to help ${selectedStudent.full_name}!`);
  };
    ...
    return (
      <div className="transfer-interface">
        <div className="goal-achieved-header">
          <h2>🎉 Goal Achieved!</h2>
          <p>{student.full_name} has reached their funding goal of KSh {student.fee_amount.toLocaleString()}</p>
          ...
          {otherStudents.map(s => {
            ...
            return (
              <div 
                ...
                onClick={() => {
                  const transferAmount = prompt(`How much would you like to transfer to ${s.full_name}?`, '1000');
              ...
              >
                ...
                <div className="student-need">Needs: KSh {remaining.toLocaleString()}</div>
                <div className="student-school">{s.school_name}</div>
              </div>
            );
          })}
        </div>
        <button className="back-btn" onClick={() => window.location.href = '/campaigns'}>
          Back to Campaigns
        </button>
      </div>
    );
  ...
  if (donationSuccess) {
    return (
      <div className="donation-success">
        ...
        <p>Your generous contribution will make a real difference in {student.full_name}'s education journey.</p>
  ...
  return (
    <div className="premium-donation-container">
      ...
      <Formik
        initialValues={{ amount: "", paymentMethod: "mpesa", phoneNumber: "", selectedBank: "", anonymous: false }}
          ...
          <Form className="donation-form">
                ...
                <div className="custom-amount-section">
                  ...
                  <div className="amount-input-group">
                    ...
                    <Field
                      ...
                      className={`amount-input ${touched.amount && errors.amount ? 'error' : ''}`}
                    />
                  </div>
              ...
              <div className="donation-section">
                ...
                <div className="payment-options">
                  <Field name="paymentMethod">
                    {({ field }) => (
                      <>
                        {[
                          { value: 'mpesa', icon: '📱', name: 'M-Pesa', desc: 'Mobile money' },
                          { value: 'card', icon: '💳', name: 'Card', desc: 'Visa/Mastercard' },
                          { value: 'bank', icon: '🏦', name: 'Bank', desc: 'Direct transfer' }
                        ].map(method => (
                          <label key={method.value} className={`payment-method ${field.value === method.value ? 'selected' : ''}`}>
                            <input
                              type="radio"
                              {...field}
                              value={method.value}
                ...
                {values.paymentMethod === 'mpesa' && (
                  <div className="phone-input">
                    ...
                    <Field 
                      ...
                      className={`phone-field ${touched.phoneNumber && errors.phoneNumber ? 'error' : ''}`}
                    />
                    <ErrorMessage name="phoneNumber" component="div" className="error" />
                    <small className="input-hint">📱 You'll receive an M-Pesa prompt on this number</small>
                  </div>
                )}
                ...
                {values.paymentMethod === 'bank' && (
                  <div className="bank-selection">
                    ...
                    <Field as="select" name="selectedBank" className={`bank-select ${touched.selectedBank && errors.selectedBank ? 'error' : ''}`}>
                    ...
                    <ErrorMessage name="selectedBank" component="div" className="error" />
                  </div>
                )}
              </div>
                ...
                <div className="privacy-options">
                  <label className="privacy-toggle">
                    ...
                    <div className="privacy-info">
                      <span className="privacy-title">🕵️ Anonymous Donation</span>
                      <span className="privacy-desc">Your name won't be shown publicly</span>
                    </div>
                  </label>
                </div>
              </div>
            ...
            <div className="donation-summary">
              <div className="summary-card">
                ...
                <div className="summary-details">
                  <div className="summary-row">
                    ...
                    <span className="amount">KSh {values.amount ? Number(values.amount).toLocaleString() : "0"}</span>
                  </div>
                  <div className="summary-row">
                    <span>Platform Fee (5%):</span>
                    <span className="fee">KSh {values.amount ? Math.round(Number(values.amount) * 0.05).toLocaleString() : "0"}</span>
                  </div>
                  <div className="summary-row total">
                    <span>Total:</span>
                    <span className="total-amount">KSh {values.amount ? Math.round(Number(values.amount) * 1.05).toLocaleString() : "0"}</span>
                  </div>
                </div>
              </div>
              ...
              <button 
                ...
                disabled={!values.amount || (values.paymentMethod === 'mpesa' && !values.phoneNumber) || (values.paymentMethod === 'bank' && !values.selectedBank)}
              >
                <span className="btn-icon">🚀</span>
                <span>Donate KSh {values.amount ? Number(values.amount).toLocaleString() : "0"}</span>
              </button>
      ...
      {showTransferModal && (
        <div className="transfer-modal">
          <div className="modal-content">
            <h3>🎉 Goal Reached!</h3>
            <p>Choose a student to transfer KSh {excessAmount.toLocaleString()} to:</p>
            <div className="student-list">
              {availableStudents.map(s => {
                const remaining = s.fee_amount - (s.amount_raised || 0);
                return (
                  <div key={s.id} className="student-option" onClick={() => handleTransfer(s)}>
                    <div className="student-name">{s.full_name}</div>
                    <div className="student-need">Needs: KSh {remaining.toLocaleString()}</div>
                    <div className="student-school">{s.school_name}</div>
                  </div>
